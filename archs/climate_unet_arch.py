...
Norm = nn.BatchNorm2d  # 改为普通BatchNorm，避免分布式依赖
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(x, dict):
            x_input = x['lq']
            hgt_input = x.get('hgt', None) if self.add_hgt else None
        else:
            x_input = x
            hgt_input = None

        x1 = self.inc(x_input)
        if self.add_hgt and hgt_input is not None:
            x_hgt = self.hgt_net(hgt_input)
