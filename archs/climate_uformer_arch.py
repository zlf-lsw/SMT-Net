...
class ClimateUformer(nn.Module):
    ...
    def forward(self, x_in, mask=None):
        ...
        if self.add_hgt and hgt is not None:
            # hgt 输入是 [2, 64, 64]（全集地形），需要匹配batch和通道
            # 取第一个样本的地形并扩展到batch；下采样到与特征相同分辨率（16x16）
            if hgt.dim() == 3 and hgt.shape[0] == 2:
                hgt_map = hgt.unsqueeze(0).repeat(x.shape[0], 1, 1, 1)  # [B,2,64,64]
            else:
                hgt_map = hgt
            hgt_feat = self.hgt_net(hgt_map)
